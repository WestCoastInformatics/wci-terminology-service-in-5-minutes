...
public class ResultListMapping {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResultListMapping
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResultListMapping.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResultListMapping is not found in the empty JSON string", ResultListMapping.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ResultListMapping.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResultListMapping` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResultListMapping.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayitems.size(); i++) {
        Mapping.validateJsonElement(jsonArrayitems.get(i));
      };
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResultListMapping.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResultListMapping' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResultListMapping> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResultListMapping.class));

       return (TypeAdapter<T>) new TypeAdapter<ResultListMapping>() {
           @Override
           public void write(JsonWriter out, ResultListMapping value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResultListMapping read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of ResultListMapping given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of ResultListMapping
  * @throws IOException if the JSON string is invalid with respect to ResultListMapping
  */
  public static ResultListMapping fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResultListMapping.class);
  }

 /**
  * Convert an instance of ResultListMapping to an JSON string
  *
  * @return JSON string
  */
