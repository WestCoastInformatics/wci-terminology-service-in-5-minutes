...
import org.openapitools.client.model.ConceptRelationship;
...
public class ResultListConceptRelationship {
  ...
  * @throws IOException if the JSON Element is invalid with respect to ResultListConceptRelationship
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ResultListConceptRelationship.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ResultListConceptRelationship is not found in the empty JSON string", ResultListConceptRelationship.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ResultListConceptRelationship.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ResultListConceptRelationship` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ResultListConceptRelationship.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayitems.size(); i++) {
        ConceptRelationship.validateJsonElement(jsonArrayitems.get(i));
      };
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ResultListConceptRelationship.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ResultListConceptRelationship' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ResultListConceptRelationship> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ResultListConceptRelationship.class));

       return (TypeAdapter<T>) new TypeAdapter<ResultListConceptRelationship>() {
           @Override
           public void write(JsonWriter out, ResultListConceptRelationship value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ResultListConceptRelationship read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of ResultListConceptRelationship given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of ResultListConceptRelationship
  * @throws IOException if the JSON string is invalid with respect to ResultListConceptRelationship
  */
  public static ResultListConceptRelationship fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ResultListConceptRelationship.class);
  }

 /**
  * Convert an instance of ResultListConceptRelationship to an JSON string
  *
  * @return JSON string
  */
